...
class SingInUpAct : AppCompatActivity() {
    ...
    fun onClickRegistration(view: View){
            ...
            && bindingClass.edHouseNum.text.toString() != "")
        {
            ...
            intent.putExtra(Constance.LOGIN, bindingClass.edLogin.text.toString())
            intent.putExtra(Constance.PASSWORD, bindingClass.edPassword.text.toString())
            intent.putExtra(Constance.PASSWORD, bindingClass.edName.text.toString())
            intent.putExtra(Constance.PASSWORD, bindingClass.edPhone.text.toString())
            intent.putExtra(Constance.PASSWORD, bindingClass.edHouseNum.text.toString())
            intent.putExtra(Constance.PASSWORD, bindingClass.edApartNum.text.toString())
            setResult(RESULT_OK, intent)
            finish()
